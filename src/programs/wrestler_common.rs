...
pub extern fn aw_draw_result(string: *const u8, status: u32, extra_data: u32) {
    ...
    if status & 0xFF == 0 {
        ...
        log!("FAIL");
        if !skip_flags {
            if status & 1 != 0 { log!(" C-flag"); }
            if status & 2 != 0 { log!(" N-flag"); }
            if status & 4 != 0 { log!(" V-flag"); }
            if status & 8 != 0 { log!(" Z-flag"); }
            if status & 0x40 != 0 { log!(" Q-flag"); }
        }
        if status & 0x10 != 0 { log!(" Rd-val"); }
        if status & 0x20 != 0 {
            log!(" Rn-val(0x{:08X})", unsafe { aw_rn_val });
        }
        if status & 0x80 != 0 {
            log!(" mem-val(0x{:08X})", unsafe { aw_mem_val });
        }
        log!("");
    }
}
...
#[no_mangle]
pub extern fn tw_draw_text(string: *const u8, x: u32, y: u32, color: u32) {
